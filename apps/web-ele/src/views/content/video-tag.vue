...
defineOptions({ name: "ContentVideoTag" });

/** 视频固定走 tag.content_type = 1(影片) */
const VIDEO_TYPE = 1;
...
async function handleDelete(row: TagApi.Item) {
  await ElMessageBox.confirm(
    `确认删除标签「${row.name}」? 已打过该标签的视频不会自动清理。`,
    "提示",
    { type: "warning" },
  );
    ...
    <ElDialog v-model="dialog" :title="isEdit ? '编辑标签' : '新增标签'" width="520px">
      <ElForm ref="formRef" :model="form" :rules="rules" label-width="90px">
        <ElFormItem label="标签名" prop="name">
          <ElInput v-model="form.name" placeholder="如: 爱情 / 动作" maxlength="64" />
        </ElFormItem>
        ...
        <ElFormItem label="状态">
          ...
          <span class="ml-2 text-xs text-gray-400">禁用后视频表单不再出现该选项</span>
        </ElFormItem>
      </ElForm>
      <template #footer>
