...
import {
  createCartoonCategoryApi,
  deleteCartoonCategoryApi,
  getCartoonCategoryListApi,
  updateCartoonCategoryApi,
} from "#/api/core/cartoon-category";
import type { VideoCategoryApi } from "#/api/core/video-category";

defineOptions({ name: "ContentCartoonCategory" });
...
async function fetchList() {
  loading.value = true;
  try {
    const res = await getCartoonCategoryListApi({
...
async function handleSave() {
  ...
  try {
    if (isEdit.value) {
      await updateCartoonCategoryApi(form.id, body);
      ElMessage.success("已保存");
    } else {
      await createCartoonCategoryApi(body);
...
async function handleDelete(row: VideoCategoryApi.Item) {
  await ElMessageBox.confirm(
    `确认删除分类「${row.name}」? 已用该分类的动漫不会自动改掉。`,
    "提示",
    { type: "warning" },
  );
  await deleteCartoonCategoryApi(row.id);
    ...
    <ElDialog v-model="dialog" :title="isEdit ? '编辑分类' : '新增分类'" width="520px">
      <ElForm ref="formRef" :model="form" :rules="rules" label-width="90px">
        <ElFormItem label="分类名" prop="name">
          <ElInput v-model="form.name" placeholder="如: 番剧 / 剧场版" maxlength="32" />
        </ElFormItem>
        <ElFormItem label="类型">
        ...
        <p class="mb-3 ml-[90px] text-xs text-gray-400">
          普通分类会出现在 H5 顶栏, 也可给动漫打标; 新更/推荐/榜单只占栏位, 不作为作品分类。
        </p>
