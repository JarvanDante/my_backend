...
const list = ref<VideoCategoryApi.Item[]>([]);
...
async function fetchList() {
  loading.value = true;
  try {
    const res = await getDouyinCategoryListApi({
...
function openEdit(row: VideoCategoryApi.Item) {
  isEdit.value = true;
  Object.assign(form, {
    id: row.id,
    name: row.name,
...
async function handleSave() {
  ...
  try {
    if (isEdit.value) {
      await updateDouyinCategoryApi(form.id, body);
      ElMessage.success("已保存");
    } else {
      await createDouyinCategoryApi(body);
...
async function handleDelete(row: VideoCategoryApi.Item) {
  await ElMessageBox.confirm(
    `确认删除分类「${row.name}」? 已用该分类的抖音不会自动改掉。`,
    "提示",
    { type: "warning" },
  );
  await deleteDouyinCategoryApi(row.id);
    ...
    <ElDialog v-model="dialog" :title="isEdit ? '编辑分类' : '新增分类'" width="520px">
      <ElForm ref="formRef" :model="form" :rules="rules" label-width="90px">
        <ElFormItem label="分类名" prop="name">
          <ElInput v-model="form.name" placeholder="如: 生活 / 探店" maxlength="32" />
        </ElFormItem>
        <ElFormItem label="类型">
        ...
        <p class="mb-3 ml-[90px] text-xs text-gray-400">
          普通分类给作品打标。关注 / 喜欢是用户行为流，不要做成分类；热门可用「榜单」。
        </p>
        ...
        <ElFormItem label="状态">
          ...
          <span class="ml-2 text-xs text-gray-400">禁用后不再出现在作品表单</span>
        </ElFormItem>
      </ElForm>
      <template #footer>
