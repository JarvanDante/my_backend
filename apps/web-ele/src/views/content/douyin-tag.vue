...
defineOptions({ name: "ContentDouyinTag" });

/** 抖音固定走 tag.content_type = 2 */
const DOUYIN_TYPE = 2;
...
async function handleDelete(row: TagApi.Item) {
  await ElMessageBox.confirm(
    `确认删除标签「${row.name}」? 已打过该标签的抖音不会自动清理。`,
    "提示",
    { type: "warning" },
  );
    ...
    <ElDialog v-model="dialog" :title="isEdit ? '编辑标签' : '新增标签'" width="520px">
      <ElForm ref="formRef" :model="form" :rules="rules" label-width="90px">
        <ElFormItem label="标签名" prop="name">
          <ElInput v-model="form.name" placeholder="如: 学生 / 留学生" maxlength="64" />
        </ElFormItem>
        ...
        <ElFormItem label="状态">
          ...
          <span class="ml-2 text-xs text-gray-400">禁用后抖音表单不再出现该选项</span>
        </ElFormItem>
      </ElForm>
      <template #footer>
