...
async function fetchList() {
  loading.value = true;
  try {
    const res = await getVideoCategoryListApi({
...
async function handleSave() {
  ...
  try {
    if (isEdit.value) {
      await updateVideoCategoryApi(form.id, body);
      ElMessage.success("已保存");
    } else {
      await createVideoCategoryApi(body);
...
async function handleDelete(row: VideoCategoryApi.Item) {
  await ElMessageBox.confirm(
    `确认删除分类「${row.name}」? 已用该分类的视频不会自动改掉。`,
    "提示",
    { type: "warning" },
  );
  await deleteVideoCategoryApi(row.id);
    ...
    <ElDialog v-model="dialog" :title="isEdit ? '编辑分类' : '新增分类'" width="520px">
      <ElForm ref="formRef" :model="form" :rules="rules" label-width="90px">
        <ElFormItem label="分类名" prop="name">
          <ElInput v-model="form.name" placeholder="如: 电影 / 短剧" maxlength="32" />
        </ElFormItem>
        <ElFormItem label="类型">
        ...
        <p class="mb-3 ml-[90px] text-xs text-gray-400">
          普通分类会出现在 H5 顶栏, 也可给视频打标; 新更/推荐/榜单只占栏位, 不作为作品分类。
        </p>
