...
import { getCartoonCategoryListApi } from "#/api/core/cartoon-category";
...
defineOptions({ name: "ContentCartoonModule" });

const TAG_TYPE = 3;
const DEFAULT_POS = "cartoon_home";
const posOpts = [{ label: "动漫首页", value: DEFAULT_POS }];
const styleOpts = [
...
async function fetchCategories() {
  const res = await getCartoonCategoryListApi({ status: "1", page: 1, size: 200 });
...
async function fetchList() {
  loading.value = true;
  try {
    const res = await cartoonModuleApi.list({
...
async function handleSave() {
  ...
  try {
    if (isEdit.value) {
      await cartoonModuleApi.update(form.id, body);
      ElMessage.success("已保存");
    } else {
      await cartoonModuleApi.create(body);
...
async function handleDelete(row: MediaModuleApi.Item) {
  ...
  await cartoonModuleApi.remove(row.id);
    ...
    <ElDialog v-model="dialog" :title="isEdit ? '编辑模块' : '新增模块'" width="560px">
      <ElForm ref="formRef" :model="form" :rules="rules" label-width="100px">
        <ElFormItem label="名称" prop="name">
          <ElInput v-model="form.name" placeholder="如: 最新 / 推荐" maxlength="64" />
        </ElFormItem>
