...
} from "#/api/core/post-category";
import type { VideoCategoryApi } from "#/api/core/video-category";

defineOptions({ name: "CommunityPostCategory" });
...
async function fetchList() {
  loading.value = true;
  try {
    const res = await getPostCategoryListApi({
...
async function handleSave() {
  ...
  try {
    if (isEdit.value) {
      await updatePostCategoryApi(form.id, body);
      ElMessage.success("已保存");
    } else {
      await createPostCategoryApi(body);
...
async function handleDelete(row: VideoCategoryApi.Item) {
  await ElMessageBox.confirm(
    `确认删除分类「${row.name}」? 已用该分类的帖子不会自动改掉。`,
    "提示",
    { type: "warning" },
  );
  await deletePostCategoryApi(row.id);
    ...
    <ElDialog v-model="dialog" :title="isEdit ? '编辑分类' : '新增分类'" width="520px">
      <ElForm ref="formRef" :model="form" :rules="rules" label-width="90px">
        <ElFormItem label="分类名" prop="name">
          <ElInput v-model="form.name" placeholder="如: 活动专区 / 日常专区" maxlength="32" />
        </ElFormItem>
        <ElFormItem label="类型">
