...
defineOptions({ name: "CommunitySection" });

/** 帖子板块固定走 tag.content_type = 6 */
const POST_TYPE = 6;
...
const rules = {
  name: [{ required: true, message: "板块名必填", trigger: "blur" }],
};
...
async function handleDelete(row: TagApi.Item) {
  await ElMessageBox.confirm(
    `确认删除板块「${row.name}」? 已发帖上的该板块名不会自动清理。`,
    "提示",
    { type: "warning" },
  );
  ...
  <div class="p-5">
    <ElCard shadow="never">
      <div class="mb-4 flex flex-wrap items-center gap-2">
        ...
        <ElButton type="primary" @click="openCreate">新增板块</ElButton>
      </div>

      <ElTable v-loading="loading" :data="list" border stripe>
        <ElTableColumn prop="id" label="ID" width="80" />
        <ElTableColumn prop="name" label="板块名" min-width="160" />
    ...
    <ElDialog v-model="dialog" :title="isEdit ? '编辑板块' : '新增板块'" width="520px">
      <ElForm ref="formRef" :model="form" :rules="rules" label-width="90px">
        <ElFormItem label="板块名" prop="name">
          <ElInput v-model="form.name" placeholder="如: 日常分享 / 活动公告" maxlength="64" />
        </ElFormItem>
        ...
        <ElFormItem label="状态">
          ...
          <span class="ml-2 text-xs text-gray-400">禁用后发帖页不再出现该板块</span>
        </ElFormItem>
      </ElForm>
      <template #footer>
